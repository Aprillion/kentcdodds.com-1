...
for that purpose (like `toHaveBeenCalledTimes`). Jest 27 has an assertion called 
[`toHaveBeenNthCalledWith`](https://jestjs.io/docs/expect#tohavebeennthcalledwithnthcall-arg1-arg2-),
so we could have avoided our `forEach` if this blog was written in 2021 and not
2018, but I think it's ok as it is (and luckily we implemented our own metadata
collection in the same way Jest does, so we don't need to change that assertion.
Fancy that!).
